...
struct LedCmd {
  ...
  char    target[16];  // targetCount (เป้า/กะ / ค้างผลิต)
  uint8_t r, g, b;
  uint8_t fontSize;
  int     speed;   // 0 = ไม่เปลี่ยน
  bool    showClock; // true = แสดง HH:MM:SS (เว็บล้างป้าย / ไม่มี state)
};
...
void syncNtpIfNeeded() {
  ...
  Serial.println("[NTP] Sync timeout — clock may show --:--:--");
}
...
// ======================================================================
//  applyDefaultLedVisual — เมื่อเว็บไม่มี state → แสดงนาฬิกา HH:MM:SS
// ======================================================================
...
bool connectBestWifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    delay(1500);
    syncNtpIfNeeded();
    // ดึง led_state จาก Laravel ให้ป้ายตรงกับหน้าเว็บ (มี state → แสดงทันที, ไม่มี → นาฬิกา)
